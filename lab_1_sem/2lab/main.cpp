...
int main(){
    ...
    for(int i=0;i<50;i++){
      double x=rand()%1000;
      double y=rand()%1000;
      double vx=(1+rand()%8)/100.0;
      double vy=(1+rand()%8)/100.0;
       array[i][0]=vx;
      array[i][1]=vy;
      p.push_back(PARTICLE(x,y));
    }
 RenderWindow window(VideoMode(1000, 1000), "Brownian Motion");
 while (window.isOpen())
    {
        Event event;
...
j++;
         ...
         if(i->st.getPosition().x>990){
                    array[j][0]=- array[j][0];
                 }
        else if(i->st.getPosition().x<10){

                    array[j][0]=-array[j][0];
                 }
                    else if(i->st.getPosition().y<10){
                    array[j][1]=-array[j][1];
                 }
         else if(i->st.getPosition().y>950){
                        array[j][1]=-array[j][1];
                 }
             ...
             for(vector<PARTICLE>::iterator k=p.begin();k!=p.end();++k){
                 ...
                 if((getDistance(i->st.getPosition().x,i->st.getPosition().y,k->st.getPosition().x,k->st.getPosition().y)<=20+0.1)&&(i!=k)){
